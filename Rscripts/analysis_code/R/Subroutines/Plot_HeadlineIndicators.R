...
Plot_HeadlineIndicators <- function()
{
  ...
  Rawdata <- list("DE" = t( read.csv(paste0(path$processed,"/IRPextraction/IRPextraction_RHS_",job$year,".csv"),header = FALSE) ),
  ...
  Rawdata <- data.frame("DE" = colSums( Rawdata$DE %*% R2M ),
                        "IM" =  colSums( Rawdata$IM %*% R2M ),
                        "EX" = colSums( Rawdata$EX %*% R2M ),
                        "DMC" = colSums( Rawdata$DMC %*% R2M ) )
  ...
  WSA[["path"]] <- paste0(path$IE_processed,"/WSA/WSA_",job$year,"_",WSA$Settings$FeedName,".csv")
  
  WSA[["data"]] <- lapply(X = WSA$path, FUN = read.csv)
  
  Steel <- list("BOF" = WSA$data[[4]],
                "EAF" = WSA$data[[5]] )
  
  Rawdata[Steel$BOF$base,"BOF"] <- Steel$BOF$Quantity
  Rawdata[Steel$EAF$base,"EAF"] <- Steel$EAF$Quantity
  ...
  SI_dir <- paste0( path$output,"/SI" )  # Set path to folder for SI
  if( !dir.exists(SI_dir) ) dir.create( SI_dir )  # Create empty folder for SI
  
  write.csv(SI, file = paste0(SI_dir,"/ew-MFA Headline Indicators.csv" ), row.names = FALSE )  
  ...
  write.csv(SI, file = paste0(SI_dir,"/Trade flow data SI.csv" ), row.names = FALSE )  
