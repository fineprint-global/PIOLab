...
Plot_Ratios <- function()
{
  ...
  realized$Oxygen <- SUT$v[ base$input %>% filter(Name == "Air") %>% pull(Code), Code_sel$BF ]
  ...
  realized[,c("Ore","Coke","Oxygen","Flux","Gas","Slag")] <- round( realized[,c("Ore","Coke","Oxygen","Flux","Gas","Slag")] / realized$Total, digits = 3)
  
  # Select WSA regions and reorder for plot
  realized <- realized[rawdata$PigIron$base,c("region","Ore","Coke","Oxygen","Flux","Gas","Slag")]
  ...
  ggsave(path = path$output, 
         filename = "Ratios_BlastFurnace.png",
         width = 8, height = 5 )
  
  ## Write ratio data to folder for SI in paper
  
  write.xlsx( realized, file = paste0(path$output,"/SI/Data for ratio realization.xlsx") )
  ...
  # Save plot 
  ggsave(path = path$output, 
         filename = "Ratios_BasicOxygenFurnace.png",
         width = 14, height = 8 )
  ...
  # Save plot 
  ggsave(path = path$output, 
         filename = "Ratios_ElectricArcFurnace.png",
         width = 14, height = 7 )
