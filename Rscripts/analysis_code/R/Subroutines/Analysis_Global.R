...
Analysis_Y <- function(reg)
{
  ...
  write.xlsx( List, paste0(path$output,"/SI/Data for Figure 7-BC.xlsx"), rowNames = FALSE )
  ...
  scale <- 1200   # This factor scales the secondary y-axis in a way that bot y-axis overlap in terms of grid lines (ticks)
  ...
  ggplot(data = df) +
    ...
    geom_point(size = 5, aes(x = Product, y = ( value_m * scale ),
                   shape = Multiplier,
                   color = Multiplier) ) +
    scale_y_continuous(breaks = c(300,600,900,1200),sec.axis = sec_axis(~./scale,
  ...
  ggsave(path = path$output, 
         filename = paste0("Dashboard_Global_1_.png"),
         width = 9, height = 8 )  
  ...
  ggplot(data = direct) +
    ...
    guides( fill = guide_legend(order = 1),
            shape = guide_legend(order = 0) ) +
    scale_y_continuous(breaks = c(150, 300, 450, 600 ) )
  
  # Save plot: 
  ggsave(path = path$output, 
         filename = paste0("Dashboard_Global_2_.png"),
         width = 9, height = 8 )

  
  ## Create table with all numbers for paper annex
  
  annex <- cbind( "Item" = "Final consumption", direct[,1:3] )
