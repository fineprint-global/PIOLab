...
# Select Flat Rolled Products
item_page <- items[[8]]$page
...
data <- SE_LogRegression(data,0.2,0.05)
...
{ 
  ...
  ALANG <- add_row(ALANG,'1' = paste0("DataFeed Flat Rolled Products ",reg_name), 
                   Value = value,'Row parent' = reg_num,'Row child' = "1",'Row grandchild' = "43-58",
                   'Column parent' = reg_num,'Column child' = "2",'Column grandchild' = "45-125",S.E. = SE)
}
...
ALANG$Years <- "1"
ALANG$Margin <- "1"
ALANG$Coef1 <- as.character(1/(length(43:58)*length(45:125)))
