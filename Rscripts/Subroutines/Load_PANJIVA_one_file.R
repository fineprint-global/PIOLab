...
#CREATE COUNTRY DATA
######################################################################
create_country_data_one_file <- function(filename, country){
  ###############
  #load whole dataset
  data <- load_all_data(filename)
  #4 digit HS code without "." is in HS. Code column
  HS_Code_4dig_number <- paste0(substr(HS_4dig_now,start = 1, stop = 2), substr(HS_4dig_now,start = 4,stop = 5))
  data_filtered <- 
    data %>%
    subset(grepl(HS_Code_4dig_number, HS.Code, fixed = TRUE)) %>%
    subset(Shipment.Origin==countries[countries==country])
  ###############
...
details_Port_Lad <- function(filename){
  ...
  for(x in 1:length(countries)){
    assign(paste0(short[x],HS),create_country_data_one_file(filename,countries[x]))
  ...
  for (x in 1:5){
    ...
    #How often in multiple HS code
    MH[1] <- rnames[4]
    MH[2] <- i
    MH[x+2] <- sum(str_count(get(paste0(short[x],HS))$HS.Code, pattern = ";")+1>1)
    
    #How often in single HS code
    SH[1] <- rnames[4]
    SH[2] <- i
    SH[x+2] <- nrow(get(paste0(short[x],HS))) - sum(str_count(get(paste0(short[x],HS))$HS.Code, pattern = ";")+1>1)
  }
  
  ###############
  #create data frame
  sheet1 <- data.frame(t(NU[-1]))
  sheet2 <- data.frame(t(PN[-1]))
  sheet3 <- data.frame(t(PS[-1]))
  sheet4 <- data.frame(t(PS[-1]))
  sheet5 <- data.frame(t(PS[-1]))
  ...
  for (i in current_prods){
    ...
    sheet1.1 <- data.frame(t(NU[-1]))
    sheet2.1 <- data.frame(t(PN[-1]))
    sheet3.1 <- data.frame(t(PS[-1]))
    sheet4.1 <- data.frame(t(MH[-1]))
    sheet5.1 <- data.frame(t(SH[-1]))
    
    names(sheet1.1) <- names(sheet1)
    names(sheet2.1) <- names(sheet1)
    names(sheet3.1) <- names(sheet1)
    names(sheet4.1) <- names(sheet1)
    names(sheet5.1) <- names(sheet1)
    
    sheet1 <- rbind(sheet1,sheet1.1)
    sheet2 <- rbind(sheet2,sheet2.1)
    sheet3 <- rbind(sheet3,sheet3.1)
    sheet4 <- rbind(sheet4,sheet4.1)
    sheet5 <- rbind(sheet5,sheet5.1)
  }
  return(list(sheet1, sheet2, sheet3, sheet4, sheet5))
}
####################################################################
#PORTS UNLADING
####################################################################
